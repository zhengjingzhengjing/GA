...
let和const不像var，不存在变量提升，如果不定义先使用，var会提示undefined，而let和const会报错，提示未初始化

let和const在同一个作用域内不能重复定义

var是函数作用域，而let和const是块级作用域
...
set想相当于数组（add），map相当于对象，（键值对，set，for of取值很方便）
